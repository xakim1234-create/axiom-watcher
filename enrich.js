...
const ENRICH_MINUTES = +(process.env.ENRICH_MINUTES ?? 5);          // сколько минут выдерживать перед проверкой
const ENRICH_INTERVAL_MS = +(process.env.ENRICH_INTERVAL_MS ?? 3000); // пауза между монетами
...
const ENRICH_BACKLOG_CAP = +(process.env.ENRICH_BACKLOG_CAP ?? 200);
...
// ===== core compute (перенос твоей логики из консоли) =====
async function computeSnapshot(mint) {
  ...
  if (creator) {
    try {
      ...
      // не критично — просто оставим t0 = createdTs
      console.warn(`[${mint}] trades/batch failed: ${e.message || e}`);
    }
  }
  ...
  // 4) ATH 5m & all
  let ath5 = -Infinity, ath5Ts = null;
  let athAll = -Infinity, athAllTs = null;
  ...
  const currentMc = Number.isFinite(repo.usd_market_cap)
    ? repo.usd_market_cap
    : lastPrice * supplyDisplay;
  ...
  return {
    ...
    p0,
    minStart,
    ath5: Number.isFinite(ath5) ? ath5 : null,
    ath5Ts,
    athAll: Number.isFinite(athAll) ? athAll : null,
    athAllTs,
    currentMc,
    trades: {
      win1, win5, winAll
...
async function pickOneClient(client) {
  // выбираем одну монету, старше ENRICH_MINUTES и готовую к обработке
  const q = `
    select ca
    from pump_tokens
...
async function setStatus(client, ca, status, extra = {}) {
  ...
  await client.query(
    `update pump_tokens set ${sets} where ca = $1`,
    [ca, ...vals]
  );
}

async function saveSnapshot(client, ca, snap) {
  const s = snap;
  const q = `
...
async function updateTokenMeta(client, ca, snap) {
  const q = `
    update pump_tokens
    ...
    where ca = $1
  `;
  await client.query(q, [ca, snap.creator, snap.createdTs, snap.decimals]);
}
...
async function processOne(ca) {
  ...
  try {
    ...
    const row = await client.query(
      `select enrich_status from pump_tokens where ca = $1 for update`,
      [ca]
    );
    if (row.rowCount === 0) {
    ...
    // 429 — ставим отложенный повтор + экспоненциальный бэкофф
    if (e.status === 429 || /HTTP 429/.test(msg)) {
      global429Cooldown = Math.min(
        global429Cooldown ? global429Cooldown * 2 : ENRICH_COOLDOWN_429_MS,
        ENRICH_COOLDOWN_MAX_MS
      );
      const retryAt = new Date(Date.now() + global429Cooldown);
      await setStatus(client, ca, 'err', {
        last_enrich_error: `429 rate limit`,
        next_enrich_at: retryAt
      });
      console.warn(`[429] ${ca} cooldown=${global429Cooldown}ms`);
      return { err: true, rate: true };
    }

    // другая ошибка — быстрый повтор через минуту
    const retryAt = new Date(Date.now() + 60_000);
    await setStatus(client, ca, 'err', {
      last_enrich_error: msg.slice(0, 400),
      next_enrich_at: retryAt
    });
...
async function loop() {
  ...
  while (true) {
    ...
    try {
      ...
      // если ранее поймали 429 — глобальная пауза
      if (global429Cooldown) {
        await sleep(global429Cooldown);
      } else {
